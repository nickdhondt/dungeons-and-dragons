...
if(empty($_POST["data"])){
    ...
    //Check if the post data is true.
    if($post_data == true){
            ...
            if($user_data["permission_type"] === "1"){
                if(get_turn() >= 0){
                    ...
                    if($succes){
                        ...
                        //re-evaluate is not needed if the admin is next
                        if($next_turn != 0){
        ...
        $errors[] = "De server kreeg onverwachte data toe. Gelieve de request te controleren.";
    }
}
